...
# File based on https://pytorch-geometric.readthedocs.io/en/latest/_modules/torch_geometric/nn/models/tgn.html

class MTGNMemory(torch.nn.Module):
    ...
    def __init__(self, num_nodes: int, raw_msg_dim: int, memory_dim: int,
                 time_dim: int, idx_dim: int, message_module: Callable,
        ...
        self.use_idx_enc = (idx_dim != -1)
        if self.use_idx_enc:
            self.idx_enc = TimeEncoder(idx_dim)

        self.gru = GRUCell(aggregator_module.out_channels, memory_dim)
    ...
    def _get_updated_memory(self, n_id: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        msg_s, t_s, src_s, dst_s, raw_msg_s = self._compute_msg(n_id, self.msg_s_store,
                                                     self.msg_s_module)

        # Compute messages (dst -> src).
        msg_d, t_d, src_d, dst_d, raw_msg_d = self._compute_msg(n_id, self.msg_d_store,
                                                     self.msg_d_module)
        ...
        assoc_dst = self._assoc[src_d]

        aggr = self.aggr_module(
            msg_s, msg_d,
            assoc_src, assoc_dst,
            t_s, t_d,
            n_id.size(0))
        ...
        past_mem = self.memory[n_id]
        memory = self.gru(aggr, past_mem)
    ...
    def _compute_msg(self, n_id: Tensor, msg_store: TGNMessageStoreType,
        ...
        if self.use_idx_enc:
            src_enc, dst_enc = self.idx_enc(src.to(raw_msg.dtype)), self.idx_enc(dst.to(raw_msg.dtype))
            msg = msg_module(self.memory[src], self.memory[dst], raw_msg, t_enc, src_enc, dst_enc)
        else:
            msg = msg_module(self.memory[src], self.memory[dst], raw_msg, t_enc)

        return msg, t, src, dst, raw_msg
...
class LastAggregator(torch.nn.Module):
    ...
    def forward(self,
        ...
        index = torch.cat([index_src, index_dst], dim=0)
        t = torch.cat([t_src, t_dst], dim=0)
